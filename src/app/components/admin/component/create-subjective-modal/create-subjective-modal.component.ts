...
@Component({
  ...
  template: `
    <div *ngIf="isVisible" class="fixed inset-0 bg-black/30 z-50 flex justify-end" (click)="close()">
      <div class="w-full max-w-3xl bg-white shadow-2xl h-full transform transition-transform duration-300"
           [class.translate-x-0]="isVisible"
           [class.translate-x-full]="!isVisible"
           (click)="$event.stopPropagation()">
        <!-- Header -->
        <div class="flex justify-between items-center p-4 border-b">
          <h2 class="text-xl font-semibold text-gray-800">Create Subjective Question</h2>
        ...
        <div class="p-6 h-[calc(100%-200px)] overflow-y-auto">
          ...
          <div *ngIf="currentStep === 0">
            <div class="space-y-6">
              ...
              <div>
                ...
                <div class="flex gap-2">
                  ...
                  <button (click)="addTag()"
                          class="px-3 py-1 bg-gray-100 rounded-lg hover:bg-gray-200 transition-colors">Add</button>
                </div>
              </div>
            </div>
          </div>
            ...
            <div class="space-y-6">
              ...
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Must Include Keywords</label>
                <div class="flex flex-wrap gap-2 mb-2">
                  <span *ngFor="let keyword of question.mustInclude; let i = index" 
                        class="px-2 py-1 bg-green-100 text-green-700 rounded-full text-sm flex items-center">
                    {{keyword}}
                    <button (click)="removeMustInclude(i)" 
              ...
              <!-- Optional Keywords -->
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Optional Keywords</label>
                <div class="flex flex-wrap gap-2 mb-2">
                  <span *ngFor="let keyword of question.optional; let i = index" 
                        class="px-2 py-1 bg-blue-100 text-blue-700 rounded-full text-sm flex items-center">
                    {{keyword}}
                    <button (click)="removeOptional(i)" 
              ...
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Evaluation Criteria For AI</label>
                <textarea
          ...
          <div *ngIf="currentStep === 2">
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-2">Solution Template</label>
              <textarea [(ngModel)]="question.sampleAnswer" rows="10"
                        class="w-full rounded-lg border border-gray-300 focus:ring-2 focus:ring-green-500 focus:border-transparent"
                        placeholder="Enter solution template..."></textarea>
            </div>
          </div>
        </div>
        ...
        <div class="absolute bottom-0 left-0 right-0 p-4 border-t bg-white flex justify-between items-center">
          <div>
            <button (click)="saveDraft()"
                    class="px-4 py-2 text-gray-700 hover:bg-gray-100 rounded-lg transition-colors mr-2">Save as Draft</button>
          </div>
          <div class="flex gap-2">
            <button *ngIf="currentStep > 0" (click)="previousStep()"
                    class="px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors">Back</button>
            <button *ngIf="currentStep < 2" (click)="nextStep()" [disabled]="!canProceed"
                    class="px-4 py-2 bg-green-500 text-white rounded-lg hover:bg-green-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">Next</button>
            <button *ngIf="currentStep === 2" (click)="publish()" [disabled]="!isComplete"
                    class="px-4 py-2 bg-green-500 text-white rounded-lg hover:bg-green-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">Publish</button>
          </div>
        </div>
      </div>
    </div>
...
export class CreateSubjectiveModalComponent {
  ...
  get canProceed(): boolean {
    if (this.currentStep === 0) {
      return this.question.problemStatement.trim() !== '' && this.question.maxScore > 0;
    }
  ...
  addMustInclude() {
    if (this.newMustInclude.trim() && !this.question.mustInclude.includes(this.newMustInclude.trim())) {
  ...
  addOptional() {
    if (this.newOptional.trim() && !this.question.optional.includes(this.newOptional.trim())) {
