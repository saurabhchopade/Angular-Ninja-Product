        ...
        <div class="subjective-header">
          ...
          @if (!loading && questions.length > 0) {
            <div class="progress-container">
              ...
              <span class="progress-text">{{ getAnsweredCount() }} of {{ questions.length }} answered</span>
            </div>
          }
        </div>
      </div>
      ...
      <div class="subjective-content">
        @if (loading) {
          ...
          @for (question of questions; track question.questionId; let i = $index) {
            <div class="question-card" [class.answered]="isQuestionAnswered(question.questionId)">
              ...
              <div class="answer-box">
                ...
                ></textarea>
                @if (question.maxWords) {
                  <div class="word-count" [class.word-limit-exceeded]="wordCounts[question.questionId] > question.maxWords">
...
.progress-text {
  ...
  padding-right: 0.15rem; /* Reduced from 0.5rem */
    ...
    .submission-state, .success-state {
      display: flex;
      flex-direction: column;
      align-items: center;
...
export class SubjectiveSectionComponent implements OnInit, OnDestroy {
  ...
  fetchQuestions(): void {
    ...
    if (cachedQuestions) {
      ...
      this.questionService.fetchSubjectiveQuestions(assessmentId, sectionId, candidateId).subscribe({
        next: (response) => {
          if (response.code === 200) {
            ...
            localStorage.setItem('subjectiveQuestions', JSON.stringify(this.questions));
          }
          this.loading = false;
        },
        error: (error) => {
